...
class  DatatableFactory
{
    ...
    private static function filterUsers()
    {
        if (auth()->user()->hasAnyRole('administrator', 'superadministrator')) {
            return DB::table('users')->join('countries', 'country_id','=','countries.id')->join('companies', 'company_id','=','companies.id')->select('users.id as id','users.name as name','surnames','email','countries.name as country','companies.name as company');
        }
